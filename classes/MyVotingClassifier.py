...
class MyVotingClassifier:
    ...
    """
    def __init__(self, estimators: list[tuple], voting: Literal['hard', 'soft', 'S&F'] = 'hard', weights: list[float] = None, n_jobs: int = None, flatten_transform: bool = True, verbose: bool = False):
        """
        Paramètres :
    ...
    def _fit_SF(self, X_train, epsilon_arret=0.001, max_iter=100):
        ...
        predictions_matrice = np.array([estimator.predict(X_train) for estimator in self.estimators_]).T
    ...
    def _fit_SF1(self, X_train, epsilon_arret=0.001, max_iter=100):
        ...
        """
        # Obtenir toutes les prédictions
        predictions_matrice = [estimator.predict(X_train) for estimator in self.estimators_]
        n_estimators = len(self.estimators_)
